...
| v0.9.1 | Detection-filter upgrades (PR #4): uniform box filter (Huang et al. 2011) as a 3rd option; single `detectSpots()` dispatch; per-method threshold fields; real-time scrubbed-frame preview; raw-panel wheel=zoom / shift+wheel (or slider) = scrub; ROI box tracks the fit radius |
| v0.9.2 | Layout & mobile UI polish: collapsible controls panel — desktop re-open floats it as an overlay drawer over full-width canvases (no window resize on toggle) with a pin to re-dock; mobile drawer puts images first; both display windows take the data aspect ratio (equal, no letterbox) and scale to width without overflow/overlap. Fixes: pinch-zoom restored in the raw frame; non-wrapping panel headers (ellipsis + tooltip); no iOS zoom on tapping number fields; vendored libs (pako/UTIF) bracketed with BEGIN/END banners |
| v0.9.3 | Astigmatic calibration robustness: "Fix bead x,y" freezes bead centres from a composite of the calibration range (shown in the SR panel) so Calibrate only fits amplitude/σx/σy/background per frame — avoids the free-xy fit walking to a wrong lobe when the PSF flattens/rings/splits at large defocus; scrubbing overlays the frozen positions instead of re-detecting. σ_PSF/fit-radius ranges widened (5 px / 10 px). New auto data-projection in the SR panel on any load (sampled/capped, stays fast at any stack size). Fixes: z-axis auto-range no longer locks to a partial early sample on long Runs (mid-Run previews no longer write zmin/zmax, only the final render does); `useWorkers` now also triggers on total stack volume, not just per-frame size, so many-small-frame stacks parallelize. `docs/layout_bare.html` — reusable bare UI shell for future single-file projects |
