...
| Show E hist / Show E/S hist (smFRET) | `showSmfretEHist()` | no args — needs Get time traces to have produced a result with a real DD+DA pairing |
...
| `smfretMinDex` | Min D_ex (DD+DA) for E histogram | number | 0 | — | 10 | 0 |
...
<p><b>Link DD/DA/AA</b> (only enabled with <b>Alternating laser excitation?</b> checked, a DD+DA pairing already existing, AND <b>Filter SOIs</b> set to <b>DD + DA + AA</b> — that setting is what says you want independent AA confirmation at all) validates each already-paired site against an INDEPENDENT direct-acceptor-excitation localization, rather than trusting the pairing geometry's own acceptor position blindly: it runs a fresh, on-demand detect+fit pass on the direct-acceptor-excitation channel (the same Average # of frames/detection/fit settings <b>Localize SOI</b> itself uses, but not saved anywhere — a one-off comparison, re-computed fresh each click), then discards any pair whose own DA position has no real localization from that pass within 2 px of it. With <b>Filter SOIs</b> at <b>DD + DA + AA</b>, <b>Get time traces</b> stays disabled until this has succeeded at least once (see below) — a real pairing must be independently confirmed before you can trace it, not just optionally verified; with <b>Filter SOIs</b> at <b>DD + DA</b>, that requirement doesn't apply at all, since this setting already says you don't need that confirmation.</p>
