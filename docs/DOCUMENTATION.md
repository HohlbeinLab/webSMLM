...
| Pair DD + DA (smFRET) | `getSmfretPairingFromDonor()` | no args — previews+pairs the donor-channel SOI, redrawing the raw-panel histogram but leaving the reconstruction panel untouched |
| Link DD/DA/AA (smFRET) | `linkSmfretChannels()` | no args — needs ALEX on and a DD+DA pairing already done (independent of Filter SOIs) |
| Get time traces (smFRET) | `getSmfretTimeTraces()` | with ALEX on, also needs Link DD/DA/AA to have succeeded once; `getSmfretTimeTraces(true)` (the settings-toggle listeners' own call) keeps showing the current site/zoom instead of resetting to site 1 |
| Export traces (smFRET) | `exportSmfretTraces()` | no args — needs Get time traces to have produced a result first |
...
by this button, **Link DD/DA/AA**, and the live re-pair below so all three
colour the overlay identically) — the matching donor (DD) site is left in
...
rather than the unpaired majority disappearing. Once it succeeds, **Get
time traces** above
...
**Link DD/DA/AA** (`linkSmfretChannels()`, only enabled with ALEX checked
...
**Link DD/DA/AA** to the fresh pairing if it had already succeeded once,
...
| `smfretTraceChannels` | Filter SOIs | enum (`DD+DA`, `DD+DA+AA`) | — | — | — | `DD+DA` |
...
| `smfretAlignTolPx` | Align channels match tol. (px) | number (int) | 1 | 100 | 1 | 10 |
...
<p><b>Pair DD + DA</b> runs <b>Pairing (sSMLM &amp; FRET)</b>'s own candidate-fit-and-pair steps (the same computation <b>Preview pairs</b> then <b>Pair &amp; plot sSMLM</b> do) directly from here, using the current sites of interest: it fits the distance/angle window, redraws the raw (left) panel with the resulting Distances/Angles histogram (the same feedback Preview pairs itself gives), and commits the pairing (writing each site's paired acceptor position/distance) — but skips that module's own RECONSTRUCTION-panel side effects (no colour-map switch, no z-range change, no re-render), so it doesn't disturb whichever composite or time trace the reconstruction (right) panel is currently showing. With <b>Alternating laser excitation?</b> ticked, this always pairs the <b>direct donor excitation</b> channel's own sites (the channel that gives donor/acceptor, i.e. DD/DA, intensity pairs) — refuses with a log message if the SOI composite currently showing is the acceptor-excitation one instead; without ALEX there's only one channel, so it just runs. If the SOI composite is still showing when this succeeds, every site that became the ACCEPTOR (DA) side of a pair has its own ROI box and crosshair recoloured dark orange — the matching donor (DD) site keeps its usual colour, since orange marks "this is the confirmed acceptor," not just "this site paired." Every site stays visible regardless, so it's easy to see how many (and which) actually paired, rather than the unpaired majority disappearing — toggling the <b>DD+DA</b>/<b>AA</b> composite view next to the reconstruction title afterward does NOT lose this: the underlying pairing and its dark-orange marking survive a round trip to the other channel's own composite and back. Once it succeeds, <b>Get time traces</b> below automatically reads the resulting paired positions and splits DD into DD/DA, exactly as if <b>Pair &amp; plot sSMLM</b> had been clicked in the other module.</p>
<p><b>Align channels</b> is a genuinely different approach to finding the donor/acceptor geometry than <b>Pair DD + DA</b>'s own Distance/Angle-histogram fit, for a setup with clearly, spatially SEPARATED donor and acceptor regions (a dual-view/image-splitter setup) — in a field of view much wider than it is tall, random point pairs are geometrically more likely to be oriented along the LONG axis regardless of any real signal, and when that long axis happens to coincide with the true physical donor→acceptor bearing (as it does for a horizontal dual-view split), a histogram-based fit can't tell a genuine peak apart from this purely combinatorial bias. <b>Align channels</b> sidesteps histogram fitting entirely: it finds the x-position GAP between the two channels directly from where the sites of interest themselves are detected (not an assumed frame-half-width), splits into a DD region and a DA region, builds a "truth" pool for the acceptor side — DA candidates plus, with <b>Alternating laser excitation?</b> checked, a fresh independent AA localization (DA alone without ALEX, since there's no separate direct-acceptor-excitation channel to localize AA from at all) — then searches for the displacement that maximises how many DD points land near a real truth point once shifted by it (starting from a displacement of half the frame width at 180°, refined by the search itself). The winning displacement's own match set becomes the pairing directly — no separate Distance/Angle-window step needed afterward. <b>Align channels match tol. (px)</b> controls how close a shifted point must land to count as a match — kept generous by default, since a single rigid displacement is only an approximation once real optical field-dependent distortions vary the true offset slightly across the field of view.</p>
<p><b>Get time traces</b> extracts every site's intensity — its known x,y only picks which window to look at, never re-detected — in every frame of the loaded movie, then plots the resulting intensity-vs-time curve(s) in the raw (left) panel, 4:3 letterboxed like every other plot here, <b>zoomable along the x-axis</b> (mouse wheel/pinch, same as the line-profile plot; double-click to reset). With <b>Alternating laser excitation?</b> checked, this stays disabled until <b>Link DD/DA/AA</b> (above) has succeeded at least once — a safety net so you can't trace a pairing whose acceptor-channel position was never independently confirmed; without ALEX, it's available as soon as sites of interest exist, since there's no separate acceptor channel to confirm against. By default this fits a standard 2D Gaussian (seeded at the site, position free to move) at each frame; check <b>Aperture photometry (no fit)</b> to instead use a published aperture-photometry method (a circular signal disk plus a separate background annulus, background estimated by its 56th percentile) with no fit to diverge — recommended if the default trace still shows an implausible spike. <b>Set negative intensities to zero</b> (default checked) floors that method's own background-subtracted result at 0; untick it to keep a genuinely negative computed value instead, better for fitting an intensity distribution (e.g. an OFF-state population centred near zero with a real negative tail) than an artificial floor allows — a rejected fit still reports 0 either way. Toggling either of these two, or <b>Filter SOIs</b>, while a trace is already showing re-extracts every site's data with the new setting but keeps showing the SAME site at the SAME x-zoom — only a fresh click of <b>Get time traces</b> itself jumps back to site 1. While a time trace is showing, the frame scrubber below the panel is replaced by a <b>site</b> scrubber — mouse wheel (over its slider) or the bar below the panel scrolls through sites instead of frames — and a <b>Show raw frame</b>/<b>Show time trace</b> toggle next to the panel title switches back and forth between the plot and the live frame (with its own ordinary Frame scrubber) without discarding the computed traces.</p>
...
<p>The Time trace plot inset its own small <b>ROI</b> thumbnails (DD/DA/AA, shown only for whichever channel actually exists for the current site) into its top-right corner — a contrast-stretched crop of the real camera pixels around each channel's own extraction position, one FIXED representative frame per channel (the first donor-excitation frame for DD/DA, the first acceptor-excitation frame for AA), not scrubbed with the main Frame slider — each marked with a magenta crosshair at the EXACT fitted sub-pixel position used for that channel's own extraction, so you can check both that a position genuinely sits on a molecule and that the fit itself actually landed there. Update automatically when scrubbing between sites.</p>
<p>With <b>Alternating laser excitation?</b> checked, <b>Filter SOIs</b> picks whether Get time traces plots just <b>DD</b> (direct-donor-excitation frames, green) — the default — or also a rougher <b>AA</b> guess (direct-acceptor-excitation frames, blue) sampled at the SOI's own position for a site that ISN'T yet paired — before pairing there's no independently known acceptor position to sample at all, so this stays opt-in. Pairing the sites first (<b>Pair DD + DA</b> above, or <b>Pairing (sSMLM &amp; FRET)</b>'s own <b>Preview pairs</b>/<b>Pair &amp; plot sSMLM</b>, which work on Localize SOI's sites the same way they pair an ordinary sSMLM 0th/1st-order candidate) splits DD into <b>DD</b> and <b>DA</b>, AND makes a real <b>AA</b> trace available too — both read at the pair's own known acceptor position, so once paired, AA no longer needs the Filter SOIs "+AA" option at all (DD at the donor position, DA and AA at the paired acceptor position; DD/DA read during donor-excitation frames — DA needs no ALEX at all, a continuous single-laser setup still has every frame count as donor excitation — AA during direct-acceptor-excitation frames, the physically correct channel/timing for direct acceptor excitation). DA is magenta, the same colour the plot itself uses when showing just one plain, unsplit curve (the case with neither ALEX nor a pairing active).</p>
<p><b>Position donor?</b> (shown once <b>Alternating laser excitation?</b> is checked) answers the question the doubled-bearing pairing data can't answer on its own: which of the two candidate bearings — always exactly 180° apart — actually points from the donor toward the acceptor. Greyed out — before <b>Localize SOI</b> has even run, and again any time the current pairing session ends (<b>Unpair</b>, a fresh <b>Localize SOI</b>, or unchecking <b>Fix sites of interest (SOI)</b>) — until <b>Pairing (sSMLM &amp; FRET)</b>'s own angle fit has actually run at least once (from <b>Preview pairs</b> or <b>Pair DD + DA</b>) — its placeholder bearing before that has no real data behind it, so there's nothing meaningful to choose between yet. Once enabled, its two options are frozen at that fit and stay fixed while you toggle between them; picking one directly sets that module's own <b>Primary angle</b> and re-pairs — <b>Link DD/DA/AA</b> below (see next) automatically re-applies itself to the new pairing if it had already run, so a channel-linked result doesn't silently revert to unlinked. There's no automatic way to tell which of the two is physically correct ahead of time — but <b>Link DD/DA/AA</b> below doubles as an independent check: the wrong bearing typically drops its kept count to (or near) zero, since it's now checking for real acceptor signal at what's actually the donor's own position.</p>
<p><b>Link DD/DA/AA</b> (only enabled with <b>Alternating laser excitation?</b> checked, and only once a DD+DA pairing exists — independent of the Filter SOIs setting, which it doesn't read at all) validates each already-paired site against an INDEPENDENT direct-acceptor-excitation localization, rather than trusting the pairing geometry's own acceptor position blindly: it runs a fresh, on-demand detect+fit pass on the direct-acceptor-excitation channel (the same Average # of frames/detection/fit settings <b>Localize SOI</b> itself uses, but not saved anywhere — a one-off comparison, re-computed fresh each click), then discards any pair whose own DA position has no real localization from that pass within 2 px of it. With <b>Alternating laser excitation?</b> checked, <b>Get time traces</b> stays disabled until this has succeeded at least once (see below) — a real pairing must be independently confirmed before you can trace it, not just optionally verified.</p>
