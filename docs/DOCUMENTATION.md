...
than a fixed screen size — one camera pixel's on-screen width at the
current zoom (`mag×zoom`), so tracks read thin zoomed out and more
visible zoomed in. **Colour tracks by mean D** (checked by default)
