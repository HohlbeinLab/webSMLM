...
<p><b>Calibrate via AA</b> and <b>Pair (fixed window)</b> are for a setup where <b>Pair DD + DA</b>'s own auto-fit struggles to find a reliable window on its own (a dual-view/image-splitter setup, whose true separation can sit deep inside a large, smooth combinatorial background — see <a href="https://websmlm.readthedocs.io/en/latest/content/02-module-reference.html#3-pairing-ssmlm-fret" target="_blank" rel="noopener">Pairing (sSMLM &amp; FRET)</a>'s own Distances-histogram notes). AA (direct-acceptor-excitation) is a genuine, FRET-independent ground truth for where a real acceptor fluorophore sits — unlike DA, which is weak, FRET-sensitized emission riding on the donor's own excitation — so <b>Calibrate via AA</b> runs a fresh, on-demand localization on the AA channel (same settings <b>Link DD/DA/AA</b> uses) and cross-pairs it against the current donor-excitation sites of interest, giving a substantially stronger signal for the true channel geometry than DD+DA alone. It only sets <b>Pairing (sSMLM &amp; FRET)</b>'s own Distance/Angle fields and shows the resulting histogram — it does not pair anything itself. Because <b>Pair DD + DA</b> always re-fits the window from the (weaker) DD+DA signal before pairing, using it after a calibration would immediately undo it — <b>Pair (fixed window)</b> is the alternative: it pairs the current sites of interest with whatever Distance/Angle window is already set, verbatim, with no re-fit. Even with this stronger signal, treat the resulting Distance min/max as a starting point and verify by eye on the histogram — the underlying automatic fit still doesn't always converge tightly for this kind of two-region setup.</p>
