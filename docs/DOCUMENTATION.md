...
| 4 | **Save plot (SVG)** | `saveSvgBtn` | Exports a currently-shown *plot* (calibration/drift/NeNA/FRC/PCFO/line-profile/histogram) as a vector SVG instead of a raster PNG. Disabled unless at least one panel is showing a plot — never offered for the raw frame or reconstruction, which have no meaningful vector form at real localization counts. Opens the same left/right chooser as **Save image** when both panels are plots. |
