...
| Get time traces (smFRET) | `getSmfretTimeTraces()` | `getSmfretTimeTraces(true)` (the settings-toggle listeners' own call) keeps showing the current site/zoom instead of resetting to site 1 |
| Export traces (smFRET) | `exportSmfretTraces()` | no args — needs Get time traces to have produced a result first |
| Show E(S) histogram (smFRET) | `showSmfretEHist()` | no args — needs Get time traces to have produced a result with a real DD+DA pairing |
...
**Show E(S) histogram** below. Per-sample E/S use the same "each channel
